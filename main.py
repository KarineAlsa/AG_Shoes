...
def write_document(best):
    with open('best.csv', 'w', newline='') as csvfile:
        ...
        for idx, modelo_caracteristicas in enumerate(best, start=1):
            ...
            tipo, ocasion, color, talla, suela, marca, temporada = caracteristicas
            
            caracteristicas_str = (
                f"Características: Tipo {tipo}, Ocasion {ocasion}, Color {color}, "
                f"Talla {talla}, Suela {suela}, Marca {marca}, Temporada {temporada}"
            )
...
def submit():
    AG.preferencias_cliente = [tipo.get(), ocasion.get(), color.get(), talla.get(), suela.get(), marca.get(), temporada.get()]
    ...
    message = (
    f"1. Modelo: {first[0]}\n"
    f"   Características: Tipo {first[1]}, Ocasión {first[2]}, Color {first[3]}, "
    f"Talla {first[4]}, Suela {first[5]}, Marca {first[6]}, Temporada {first[7]}\n\n"
    f"2. Modelo: {second[0]}\n"
    f"   Características: Tipo {second[1]}, Ocasión {second[2]}, Color {second[3]}, "
    f"Talla {second[4]}, Suela {second[5]}, Marca {second[6]}, Temporada {second[7]}\n\n"
    f"3. Modelo: {third[0]}\n"
    f"   Características: Tipo {third[1]}, Ocasión {third[2]}, Color {third[3]}, "
    f"Talla {third[4]}, Suela {third[5]}, Marca {third[6]}, Temporada {third[7]}"
    )
...
tk.Label(root, text="Ocasión: ").pack()
ocasion = ttk.Combobox(root, values=['fiesta', 'diario', 'caminar'])
ocasion.pack()
...
tk.Label(root, text="Temporada: ").pack()
temporada = ttk.Combobox(root, values=['Primavera', 'Verano', 'Otoño', 'Invierno'])
temporada.pack()
